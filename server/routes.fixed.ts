  ...
  app.post("/api/example-queries", async (req, res) => {
    try {
      ...
      if (fileId && !dataFile) {
        return res.json({
          queries: [
            "What is the average value across all data points?",
            "Show me the highest and lowest values",
            "What are the trends over time?",
            "Compare different categories in the data",
          ],
        });
      }
      ...
      if (dataFile) {
        try {
          ...
          if (dataFile.processingResults && typeof dataFile.processingResults === 'object' && 'dataPreview' in dataFile.processingResults) {
            ...
            throw new Error("Failed to read data file and no preview available");
          }
        }
      } else {
        // Provide a default/mock dataset for demo purposes
      ...
      try {
        // ===== STEP 1: Try intelligent profiling-based query generation =====
        try {
