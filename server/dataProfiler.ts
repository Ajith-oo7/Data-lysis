...
export function profileData(data: any[]): DataProfile {
  ...
  columnNames.forEach(colName => {
    ...
    columnProfiles.push({
      ...
      uniqueValuesList: uniqueValuesList.map(v => String(v)),
      min,
      max,
      mean,
      median,
  ...
  // Count column types to infer dataset type
  const nutritionColumns = columnProfiles.filter(col => 
    col.possibleSubjectArea === 'nutrition' || 
    col.name.toLowerCase().includes('nutrition')).length;
    
  const financeColumns = columnProfiles.filter(col => 
    col.possibleSubjectArea === 'finance' || 
    col.name.toLowerCase().includes('sales')).length;
    
  const demographicColumns = columnProfiles.filter(col => 
    col.possibleSubjectArea === 'demographics').length;
  
  if (nutritionColumns > 0 && nutritionColumns >= financeColumns && nutritionColumns >= demographicColumns) {
    datasetType = 'nutritional_data';
  } else if (financeColumns > 0 && financeColumns >= nutritionColumns && financeColumns >= demographicColumns) {
    datasetType = 'sales_data';
  } else if (demographicColumns > 0) {
    datasetType = 'demographic_data';
  }
  ...
  return {
    ...
    datasetType,
    missingValuesPercentage: totalMissingValues / (data.length * columnNames.length) * 100,
  ...
  try {
    ...
    if (process.env.OPENAI_API_KEY) {
      try {
        ...
        const columnsList = profile.columns.map(col => `${col.name} (${col.dataType})`).join(', ');
        
        const missingValuesList = profile.columns
          .filter(col => col.missingPercentage > 0)
          .map(col => `${col.name}: ${col.missingPercentage.toFixed(1)}%`)
          .join(', ');
        ...
        const numericColumns = profile.columns.filter(col => col.isNumeric).map(col => col.name).join(', ');
        const categoricalColumns = profile.columns.filter(col => col.isCategorical).map(col => col.name).join(', ');
        const dateColumns = profile.columns.filter(col => col.isDate).map(col => col.name).join(', ');
...
Dataset type: ${profile.datasetType || "Unknown"}
...
export function calculateDataQualityScore(profile: DataProfile): { 
  ...
  const completeness = Math.max(0, 100 - profile.missingValuesPercentage);
  
  // 2. Uniqueness - based on duplicate rows
  const uniqueness = Math.max(0, 100 - profile.duplicateRowsPercentage);
  ...
  const dateColumns = profile.columns.filter(col => col.isDate);
  ...
  const idColumns = profile.columns.filter(col => col.isId);
  if (idColumns.length > 0) {
    const idUniqueScore = idColumns.reduce((total, col) => 
      total + (col.uniqueValues === profile.rowCount ? 100 : 0), 0) / idColumns.length;
  ...
  profile.columns.forEach(col => {
    ...
    if (col.isNumeric && !isNaN(col.min!) && !isNaN(col.max!)) {
      consistencyScore += 100;
    } 
    // Categorical columns should have reasonable number of unique values
    else if (col.isCategorical && col.uniqueValues > 0 && col.uniqueValues <= profile.rowCount / 2) {
      consistencyScore += 100;
    }
  ...
  profile.columns.forEach(col => {
    if (col.isNumeric && col.min !== undefined && col.max !== undefined && col.mean !== undefined) {
      // Check for extreme values (more than 3 times the mean)
      if (col.max > col.mean * 3 || col.min < col.mean / 3) {
...
export function determineVisualizations(profile: DataProfile): any[] {
  ...
  const numericColumns = profile.columns.filter(col => col.isNumeric && !col.isId);
  const categoricalColumns = profile.columns.filter(col => col.isCategorical && !col.isId);
  const dateColumns = profile.columns.filter(col => col.isDate);
  ...
  if (profile.datasetType === 'nutritional_data') {
    ...
    if (nameColumn) {
      ...
      nutrientColumns.slice(0, 4).forEach(nutrient => {
        const data = Array(Math.min(8, profile.rowCount)).fill(0).map((_, i) => ({
          category: `Item ${i+1}`,
      ...
      if (macros.length >= 2) {
        visualizations.push({
          ...
          data: Array(Math.min(8, profile.rowCount)).fill(0).map((_, i) => ({
            name: `Item ${i+1}`,
  ...
  else if (profile.datasetType === 'sales_data' || 
           profile.columns.some(col => col.possibleSubjectArea === 'finance')) {
...
export function generateInsights(profile: DataProfile, data: any[]): any[] {
  ...
  insights.push({
    ...
    description: `This dataset contains ${profile.rowCount} records with ${profile.columnCount} variables.`,
    recommendation: 'Review the data profile to understand the dataset structure before analysis.'
  });
  
  // Missing data insight
  if (profile.missingValuesPercentage > 0) {
    insights.push({
      title: 'Missing Data',
      description: `The dataset contains ${profile.missingValuesPercentage.toFixed(2)}% missing values.`,
      recommendation: 'Consider imputation strategies for columns with missing values.'
    });
  }
  
  // Data quality insight
  if (profile.duplicateRowsPercentage > 0) {
    insights.push({
      title: 'Duplicate Records',
      description: `Found ${profile.duplicateRowsPercentage.toFixed(2)}% duplicate records in the dataset.`,
  ...
  numericColumns.forEach(col => {
    if (col.min !== undefined && col.max !== undefined) {
      insights.push({
        ...
        description: `${col.name} ranges from ${col.min} to ${col.max} with an average of ${col.mean?.toFixed(2)}.`,
  ...
  if (profile.datasetType === 'nutritional_data') {
    const calorieColumn = profile.columns.find(col => 
      col.name.toLowerCase().includes('calorie') && col.isNumeric);
      
    if (calorieColumn) {
      insights.push({
        title: 'Caloric Content Analysis',
        description: `The average caloric content is ${calorieColumn.mean?.toFixed(2)} with a range from ${calorieColumn.min} to ${calorieColumn.max}.`,
    ...
    const sugarColumn = profile.columns.find(col => 
      col.name.toLowerCase().includes('sugar') && col.isNumeric);
      
    if (sugarColumn) {
      insights.push({
        title: 'Sugar Content Analysis',
        description: `The average sugar content is ${sugarColumn.mean?.toFixed(2)} with a range from ${sugarColumn.min} to ${sugarColumn.max}.`,
  ...
  try {
    ...
    return {
      summary: {
        ...
        columnsProcessed: profile.columnCount,
        missingValuesHandled: Math.round(profile.missingValuesPercentage * jsonData.length * profile.columnCount / 100),
        duplicatesRemoved: Math.round(profile.duplicateRowsPercentage * jsonData.length / 100),
        outliersTreated: 0,
...
export function generateQueriesFromProfile(profile: DataProfile): string[] {
  ...
  else if (profile.datasetType === 'demographic_data') {
    queries.push(
...
export async function analyzeQueryWithProfiling(query: string, data: any[], profile?: DataProfile): Promise<any> {
  try {
    ...
    if (profile.datasetType === 'nutritional_data' && hasKeyword(['sales', 'revenue', 'profit', 'customer'])) {
      canBeAnswered = false;
      answer = "This query cannot be answered as the dataset contains nutritional information, not sales or revenue data.";
    }
    else if (profile.datasetType === 'sales_data' && hasKeyword(['nutrition', 'calories', 'fat', 'protein'])) {
      canBeAnswered = false;
      ...
      if (hasKeyword(['average', 'mean', 'avg'])) {
        ...
        if (targetColumns.length > 0) {
          ...
          const descFields = profile?.columns.filter(col => 
            !col.isNumeric && !col.isId && col.name.toLowerCase() !== targetColumn.name.toLowerCase()
          ) || [];
